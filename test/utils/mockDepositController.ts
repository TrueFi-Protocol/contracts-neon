import { IDepositController__factory } from 'contracts'
...
export async function mockDepositController(
  ...
  const mockContract = await deployMockContract(
    wallet,
    IDepositController__factory.abi,
  )

  if (onDeposit) {
    await mockContract.mock.onDeposit.returns(
      onDeposit.shares,
      onDeposit?.fee ?? 0,
    )
  ...
  if (onMint) {
    ...
    await mockContract.mock.previewMint.returns(
      BigNumber.from(onMint.assets).add(onMint?.fee ?? 0),
    )
  }
