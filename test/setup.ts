...
function setupOnce() {
  ...
  async function makeLoader(): Promise<{
    loader: FixtureLoader,
    returns: FixtureReturns,
  }> {
    ...
    const loader = waffle.createFixtureLoader(
      [wallet, other, another, ...rest],
      provider,
    )
  ...
  async function loadFixture<T>(
    fixture: Fixture<T>,
  ): Promise<T & FixtureReturns> {
    // This function creates a new provider for each fixture, because of bugs
    // in ganache that clear contract code on evm_revert
    const { loader, returns } =
      currentLoader.fixture === fixture ? currentLoader : await makeLoader()
