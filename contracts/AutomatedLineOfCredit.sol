...
import {IAutomatedLineOfCredit, AutomatedLineOfCreditStatus, IPortfolio, IUtilizationRate} from "./interfaces/IAutomatedLineOfCredit.sol";
...
contract AutomatedLineOfCredit is IAutomatedLineOfCredit, ERC20Upgradeable, Upgradeable {
    ...
    function setInterestRateController(IRateComputer _interestRateController) external onlyRole(CONTROLLER_ADMIN_ROLE) {
        require(
            _interestRateController != interestRateController,
    ...
    function supportsInterface(bytes4 interfaceID) public view override(AccessControlEnumerableUpgradeable, IERC165) returns (bool) {
        return
            (interfaceID == type(IAutomatedLineOfCredit).interfaceId ||
                interfaceID == type(IPortfolio).interfaceId ||
                interfaceID == type(IUtilizationRate).interfaceId ||
